...
description: "Laravel Fortify headless authentication backend reference. Activate when implementing authentication features including login, registration, password reset, email verification, two-factor authentication (2FA/TOTP), profile updates, headless auth, authentication scaffolding, or auth guards in Laravel applications (e.g., 'implemente o login', 'adicione 2FA', 'configure a autenticação', 'add password reset')."
---

# Laravel Fortify Development (Reference Skill)

> **Reference Skill** — Provides Fortify-specific knowledge for implementation skills.
> This skill does NOT define a workflow or quality gates. Quality gates and completion
> criteria are managed by the invoking skill (task-planner, implement-task, bug-fixer).

## Integration

This skill is invoked when auth-related features are being implemented.
The invoking skill remains responsible for:

- Quality gates (see `docs/engineering/QUALITY_GATES.md`)
- Git workflow (see `docs/engineering/WORKFLOW.md`)
- Progress tracking (if applicable)

## Primary Source

Use `search-docs` with queries like `fortify authentication`, `two-factor`,
`password reset` for the most up-to-date documentation.
This skill provides a quick-reference complement to the official docs.
